...
Отдельный репозиторий **denysque/content-system** (`~/Downloads/content-system`).
Раньше был подпапкой `new app/` в репозитории invitation-gen — вынесен в свой репо.
...
```bash
vercel --prod --yes        # из корня репозитория content-system
```
...
Локально в `.env.local` (gitignored), на Vercel — в Production.
